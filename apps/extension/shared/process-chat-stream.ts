import type { OChatRequest, OMessage } from "@opengpts/types";
...
export async function processChatStream({
  ...
  >;
  experimental_onFunctionCall?: (
    chatMessages: OMessage[],
    functionCall: FunctionCall,
  ) => Promise<void | OChatRequest>;
  updateChatRequest: (chatRequest: OChatRequest | void) => void;
  getCurrentMessages: () => OMessage[];
}) {
  while (true) {
    ...
    if ('messages' in messagesAndDataOrJustMessage) {
      ...
      for (const message of messagesAndDataOrJustMessage.messages) {
        ...
        if (experimental_onFunctionCall) {
          ...
          const functionCallResponse: OChatRequest | void =
