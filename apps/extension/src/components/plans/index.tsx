...
const Plans: React.FC<{
  ...
  function handleClickPlan(plan: PlanRow) {
    ...
    if(subscription?.subscription_status !== 'active') {
      message.error('请续订后再尝试更换订阅')
      return
    }
    handleChangeSubscription(plan)
  ...
  async function handleChangeSubscription(plan: PlanRow) {
    setLoading(true);
    try {
      const res = await fetch("/api/lemon/change", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (!res.ok || resData.code !== 0) {
        message.error("handleChangeSubscription error");
        return
      }
      setSubscription({
      ...
      message.success("change subscription success")
    } catch (error) {
      message.error("error");
    } finally {
      setLoading(false);
