...
import ChatPanel, { type ChatPanelRef } from "./ChatPanel"
...
const DynamicSplitChatPanel = () => {
    ...
    const setChatPanelRef = (element: ChatPanelRef | null, index: number) => {
    ...
    const setAllChatPanelInputArea = (bool: boolean) => {
        chatPanelRefs.current.forEach((chatPanelRef) => {
            chatPanelRef?.setHideInputArea(bool)
        })
    }
            ...
            <div className='flex items-center'>
                <div className="flex flex-row items-center h-6 gap-1 bg-primary-background rounded-2xl">
                    ...
                    {panelConfigs.map(({ num, icon }) => (
                        ...
                        >
                            <img src={icon?.src || icon} className="w-6 h-6 cursor-pointer" />
                        </div>
                    ))}
                </div>
