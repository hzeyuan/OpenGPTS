...
import { Input, Button, Dropdown, Modal, Popconfirm, Typography, Space, Spin, Tag, Tooltip, notification, message } from 'antd';
...
import openaiSvg from "~assets/chatgpt3.5.svg"
...
const GPTsPanel = () => {
  ...
  const dataSource = useMemo(() => {
    ...
    return sortedGizmos.map((gizmo: Gizmo) => {
      return {
        id: gizmo.id,
  ...
  const handleRemoveGPT = async (gizmoId: Key) => {
    const result = await sendToBackground({
      name: "openai",
      body: {
        action: 'delete',
  ...
  const handleUpdatedGPT = (gizmo: Gizmo, values: any) => {
    hideModal()
    messageApi.success(t('updateSuccess', { name: gizmo.display.name }));
  }
...
链接: https://chat.openai.com/g/${gizmo.id} 欢迎大家使用。`)
  ...
  const handleBatchDelete = async (gizmoIds: Key[], onCleanSelected: Function) => {
        ...
        <ProList<{
          ...
          toolbar={{
            ...
            actions: [
              ...
              <Input.Search onSearch={(value) => { setSearchValue(value) }} placeholder="input search text" allowClear />,
