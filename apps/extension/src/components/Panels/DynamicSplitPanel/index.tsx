...
import Split, { type SplitProps } from "react-split-grid"
...
const DynamicSplitPanel: React.FC<any> = ({ panels, splitPanelNum, style }) => {
  ...
  const initGridTemplate = (count: number) => Array(count).fill("1fr").join(" 10px ")
  ...
  // 处理拖动事件
  const handleDrag: SplitProps["onDrag"] = (direction, track, gridTemplateStyle) => {
    if (direction === "row") {
