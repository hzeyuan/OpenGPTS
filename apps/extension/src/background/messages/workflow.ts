...
import convertWorkflowData from '~src/utils/convertWorkflowData'
...
const handler: PlasmoMessaging.Handler<'workflow', SendToBackgroundViaRelayRequestBody<'executeWorkflow' | 'workflow:execute' | 'workflow:stop'>, SendToBackgroundViaRelayResponseBody> = async (req, res) => {
    ...
    if (type === 'workflow:execute') {
        ...
        const {workflowData: workflowDataStr, workflowOptions} = data;
        const workflowData = JSON.parse(workflowDataStr);

        console.log('workflowData',workflowData)
        const convertedWorkflowData  =  convertWorkflowData(workflowData);
        console.log('workflowEditorRef?.current.toObject();', convertedWorkflowData);
        // try {
        //     if (!blockId) return;
